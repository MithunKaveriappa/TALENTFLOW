  ...
  useEffect(() => {
    async function loadResults() {
      try {
        ...
        const data = await apiClient.get("/assessment/results", authSession.access_token);
        setResults(data);
      } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 font-bold text-xs uppercase tracking-widest">Compiling Profile Signals...</p>
        </div>
      </div>
    );
  }
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full z-30">
        <div className="p-8 border-b border-slate-50">
          <div className="flex items-center gap-3">
             ...
             <span className="font-black text-slate-900 tracking-tighter uppercase text-lg">TalentFlow</span>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-slate-50 space-y-4">
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
             <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Trust Active</span>
             </div>
             <p className="text-[10px] text-slate-400 font-medium leading-tight">Your signals are live for 12 enterprise partners.</p>
          </div>
        ...
        {!results ? (
            <div className="min-h-[70vh] flex items-center justify-center">
                <div className="text-center p-12 bg-white rounded-[2rem] shadow-2xl shadow-slate-200/50 border border-slate-100 max-w-md w-full relative overflow-hidden group">
                    ...
                    <h1 className="text-3xl font-black text-slate-900 mb-3 tracking-tight">EVALUATION REQUIRED</h1>
                    <p className="text-slate-500 mb-10 text-sm font-medium leading-relaxed">
                        To unlock your <span className="text-slate-900 font-bold">Command Center</span> and begin matching with partners, you must first complete the baseline assessment signals.
                    </p>
            ...
            <div className="max-w-4xl mx-auto space-y-8">
              <header className="flex justify-between items-end mb-12">
                <div>
                   <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none uppercase italic">CANDIDATE COMMAND CENTER</h1>
                   <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em] mt-2">Real-time Performance Monitoring</p>
                </div>
                <div className="flex items-end gap-8">
                  <div className="text-right hidden sm:block">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Identity Status</span>
                  ...
                  >
                    <span className="text-[10px] font-bold text-slate-300 group-hover:text-red-400 uppercase tracking-widest transition-colors">Session</span>
              ...
              <div className="bg-slate-900 rounded-[2.5rem] p-10 md:p-16 text-white shadow-2xl relative overflow-hidden">
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
                  <div className="space-y-6 text-center md:text-left">
                    <span className="bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">Aggregate Talent Score</span>
                    <div className="space-y-1">
                      <h2 className="text-7xl md:text-9xl font-black tracking-tighter">{results.overall_score || 0}</h2>
                      <p className="text-slate-400 font-bold uppercase tracking-widest text-sm">Percentile Score / 100</p>
                    </div>
                    <p className="text-slate-300 max-w-sm text-sm leading-relaxed font-medium">
                      Your profile indicates <span className="text-indigo-400 font-bold">{results.overall_score >= 70 ? 'High-Fit' : results.overall_score >= 40 ? 'Moderate-Fit' : 'Emerging-Fit'}</span> for Technical Sales roles. Profile shared with 12 top-tier companies.
                    </p>
                  </div>
                  ...
                  <div className="relative">
                      <div className="h-48 w-48 md:h-64 md:w-64 rounded-full border-[1.5rem] border-slate-800 flex items-center justify-center">
                          <div className="text-center">
                              <svg className="h-12 w-12 md:h-16 md:w-16 text-indigo-500 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                              </svg>
                              <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">Trust Matrix</span>
                          </div>
                      </div>
                      {/* SVG Progress Circle */}
                      <svg className="absolute inset-0 h-full w-full -rotate-90 scale-102" viewBox="0 0 100 100">
                          <circle 
                              cx="50" cy="50" r="42" 
                              fill="transparent" 
                              stroke="currentColor" 
                              strokeWidth="4" 
                              strokeDasharray="264" 
                              strokeDashoffset={264 - (264 * (results.overall_score || 0)) / 100}
                              strokeLinecap="round"
                ...
                <div className="absolute top-0 right-0 w-1/2 h-full opacity-10 pointer-events-none">
                  ...
                  <div className="grid grid-cols-10 gap-1 h-full w-full">
                      {[...Array(100)].map((_, i) => (
                          <div key={i} className="border-[0.5px] border-white/20 h-full w-full" />
                      ))}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] p-10 border border-slate-200/60 shadow-sm overflow-hidden relative">
                 ...
                 {/* Background branding */}
                 <div className="absolute -bottom-10 -right-10 text-[120px] font-black text-slate-50 select-none">TF</div>
              </div>
            </div>
        )}
      </main>
...
function SidebarLink({ label, active = false }: { label: string, active?: boolean }) {
  return (
    <div className={`px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 transition-all cursor-pointer ${active ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-indigo-600 hover:bg-slate-50'}`}>
      <div className={`h-1.5 w-1.5 rounded-full ${active ? 'bg-white' : 'bg-slate-200 group-hover:bg-indigo-400'}`} />
      {label}
    </div>
  );
}
...
function ScoreTile({ label, score }: { label: string, score: number | undefined }) {
  ...
  return (
    <div className={`bg-white p-6 rounded-3xl border shadow-sm space-y-4 transition-colors group ${hasData ? 'border-slate-200 hover:border-indigo-200' : 'border-slate-100 opacity-60'}`}>
      <div className="flex justify-between items-start">
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-indigo-500 transition-colors">{label}</span>
        <span className="text-xl font-black text-slate-900 italic">{hasData ? `${score}%` : '--'}</span>
      </div>
...
function FeedbackItem({ title, desc, status }: { title: string, desc: string, status: string }) {
  return (
    <div className="space-y-3">
        <div className="flex items-center gap-3">
          <h4 className="font-bold text-slate-900 text-sm uppercase tracking-tight">{title}</h4>
          <span className="text-[8px] font-black uppercase bg-emerald-50 text-emerald-600 border border-emerald-100 px-2 py-0.5 rounded-md tracking-widest">{status}</span>
        </div>
