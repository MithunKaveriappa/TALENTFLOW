...
export default function AssessmentExam() {
    ...
    const handleVisibilityChange = async () => {
      if (document.visibilityState === "hidden" && !showAadhaar && !isBlocked) {
        try {
          ...
          const res = await apiClient.post("/assessment/tab-switch", {}, authSession.access_token);
          if (res.status === "blocked") {
            setIsBlocked(true);
          } else if (res.status === "warning") {
            setWarning("FINAL WARNING: Tab switching is strictly prohibited. The next switch will permanently block your account.");
          }
        } catch (err) {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const { data: blocked } = await supabase.from("blocked_users").select("*").eq("user_id", user.id).maybeSingle();
        if (blocked) {
          setIsBlocked(true);
          setIsLoading(false);
          return;
        ...
        const startRes = await apiClient.post("/assessment/start", {}, authSession.access_token);
        ...
        const qRes = await apiClient.get("/assessment/next", authSession.access_token);
  ...
  const handleNext = useCallback(async (isTimeout = false) => {
    ...
    try {
      ...
      // Get next
      const nextQ = await apiClient.get("/assessment/next", authSession.access_token);
  ...
  const handleAadhaarUpload = async () => {
    ...
    try {
      ...
      const { data: { session: authSession } } = await supabase.auth.getSession();
      if (authSession) {
        await apiClient.get("/assessment/results", authSession.access_token).catch(() => null);
      }
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 text-center shadow-2xl border-4 border-red-500">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Account Permanently Blocked</h1>
          <p className="text-slate-600 mb-6">A security violation (tab switching) was detected. Due to our high-trust policy, you are permanently disqualified from using TalentFlow.</p>
          <div className="text-xs text-slate-400 bg-slate-50 p-4 rounded-xl border mb-6">This action is irreversible.</div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        <div className="max-w-xl w-full bg-white rounded-3xl p-10 shadow-xl border border-slate-100 text-center">
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 mb-4">Assessment Complete!</h1>
          <p className="text-slate-600 mb-8 leading-relaxed">Your professional signals have been captured. To finalize your high-trust profile, please upload a clear scan of your Aadhaar card for ID verification.</p>
          ...
          <div className="relative group mb-8">
            ...
            />
            <div className={`border-2 border-dashed rounded-2xl p-10 transition-all ${aadhaarFile ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-slate-50 group-hover:bg-slate-100'}`}>
              <div className="flex flex-col items-center gap-2">
                <svg className={`h-10 w-10 ${aadhaarFile ? 'text-emerald-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
                <span className="text-sm font-semibold text-slate-700">{aadhaarFile ? aadhaarFile.name : "Select your ID Document"}</span>
          ...
          >
            {isUploading ? "Verifying..." : isFinishing ? "Profile finalized!" : "COMPLETE REGISTRATION"}
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className={`px-4 py-2 rounded-xl flex items-center gap-3 border ${timeLeft <= 10 ? 'bg-red-50 border-red-100 text-red-600 animate-pulse' : 'bg-slate-50 border-slate-100 text-slate-700'}`}>
          ...
          <button onClick={() => handleNext(true)} className="text-xs font-bold text-slate-400 hover:text-red-500 px-4 py-2 rounded-lg border border-slate-100 transition-all">SKIP</button>
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 md:py-24">
        {warning && (
          <div className="mb-8 p-4 bg-orange-50 border border-orange-100 text-orange-800 rounded-2xl flex items-start gap-4 animate-in fade-in slide-in-from-top-4">
            ...
            <button onClick={() => setWarning(null)} className="text-orange-900 ml-auto">&times;</button>
          </div>
        )}

        <div className="space-y-8">
          <div className="space-y-2">
            <span className="text-xs font-black text-indigo-500 uppercase tracking-[0.2em]">Evaluation Case</span>
